...
%%
freeSwim(ff).meanVelo = nanmedian(tmp_swim_bouts(:,7));
freeSwim(ff).peakVelo = nanmedian(tmp_swim_bouts(:,8));
freeSwim(ff).meanDuration_bouts = nanmean(tmp_swim_bouts(:,3)); %only poisson distribution
freeSwim(ff).medTurns_bouts = nanmedian(tmp_swim_bouts(:,6));
freeSwim(ff).medDistance_bouts = nanmedian(tmp_swim_bouts(:,9));
freeSwim(ff).medYaw_left = nanmedian(tmp_swim_bouts(:,10));
freeSwim(ff).medYaw_right = nanmedian(tmp_swim_bouts(:,11));
freeSwim(ff).medAngularVelocity = nanmedian(tmp_swim_bouts(:,12));
freeSwim(ff).medIBI = nanmedian(tmp_swim_bouts(:,13));
...
clearvars -except filePattern myDir myFiles freeSwim dish_repeat dish_center;
