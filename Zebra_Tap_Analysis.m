...
tapEscape(ff).datarate_Hz = datarate_Hz;

tapEscape(ff).boutAnalysis.boutLength = 150; % considering the bout length to be 200ms / 750Hz acq
tapEscape(ff).boutAnalysis.VthreshON = 1;
tapEscape(ff).boutAnalysis.BthreshOFF = 2;
...
tapEscape(ff).boutAnalysis.acquis_date.exp_type.fish_num.filters.B = filterB;
tapEscape(ff).boutAnalysis.acquis_date.exp_type.fish_num.filters.V = filterV;
tapEscape(ff).boutAnalysis.acquis_date.exp_type.fish_num.filters.F = filterF;
...
for qq = 1:size(fast_locs)
    ...
    if (fast_locs(qq) - 3*tapEscape(ff).boutAnalysis.boutLength <=0)... % get rid of any half bout in the begining
                ||(fast_locs(qq) + 3*tapEscape(ff).boutAnalysis.boutLength >=size(tmp_vel_fB,1))... % half bout in the end
                ||(any(isnan(fast_locs(qq):fast_locs(qq)+3*tapEscape(ff).boutAnalysis.boutLength)))... % remove nan-area
                ||(any(isnan(fast_locs(qq):fast_locs(qq)-3*tapEscape(ff).boutAnalysis.boutLength)))... % nan-area again
                ||tmp_vel_fB(fast_locs(qq))<= tapEscape(ff).boutAnalysis.VthreshON... %check w/ Vthresh
                ||tmp_vel_fB(fast_locs(qq))<= tapEscape(ff).boutAnalysis.BthreshOFF %check w/ Bthresh
...
for hhh = 1:size(fast_locs)
    ...
    if ttt>length(idx_tap)
        fast_locs(hhh:end)= NaN;
        break
    end
...
for mm = 1:size(fast_locs,1)
    ...
    %find start frame
    sss = 0;
    while fast_locs(mm)-sss>=2*tapEscape(ff).boutAnalysis.boutLength...
            && tmp_vel_fV(fast_locs(mm)-sss) >= tapEscape(ff).boutAnalysis.VthreshON % using narrow filter for onset
        sss = sss + 1;
    end   
    ...
    while fast_locs(mm)<=size(tmp_vel_fB,1)...
            && tmp_vel_fB(fast_locs(mm)+eee) >= tapEscape(ff).boutAnalysis.BthreshOFF % using broad filter for offset
        eee = eee + 1;
    end
...
%% 
tapEscape(ff).date = acquis_date;
tapEscape(ff).time = acquis_time;
tapEscape(ff).expType = exp_type;
tapEscape(ff).camScale = camscale_px_per_mm;
tapEscape(ff).trialLength = duration;
tapEscape(ff).rawData = tmp_data;
tapEscape(ff).totalBouts = size(tmp_swim_bouts,1);
tapEscape(ff).boutData = tmp_swim_bouts;

%%
tapEscape(ff).meanVelo = nanmedian(tmp_swim_bouts(:,7));
tapEscape(ff).peakVelo = nanmedian(tmp_swim_bouts(:,8));
tapEscape(ff).meanDuration_bouts = nanmean(tmp_swim_bouts(:,3)); %only poisson distribution
tapEscape(ff).meanTurns_bouts = nanmedian(tmp_swim_bouts(:,6));
tapEscape(ff).meanDistance_bouts = nanmedian(tmp_swim_bouts(:,9));
tapEscape(ff).meanYaw_left = nanmedian(tmp_swim_bouts(:,10));
tapEscape(ff).meanYaw_right = nanmedian(tmp_swim_bouts(:,11));
tapEscape(ff).meanAngularVelocity = nanmedian(tmp_swim_bouts(:,12));
tapEscape(ff).meanIBI = nanmedian(tmp_swim_bouts(:,13));
...
clearvars -except filePattern myDir myFiles tapEscape dish_repeat dish_center;
